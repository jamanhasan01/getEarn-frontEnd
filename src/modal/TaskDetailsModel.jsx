      ...
      {showTaskModel && (
        <div className="fixed left-0 top-0 w-full h-full z-50  bg-black/90">
          <div className="fixed left-[50%] top-[50%] w-3/6 bg-white -translate-x-[50%] -translate-y-[50%] p-3 text-gray-800">
            ...
            <button className="text-2xl font-semibold text-right " onClick={() => setshowTaskModel(false)}>X</button>
            </div>
            <div className="flex flex-col gap-3">
               <div className="p-2 border rounded-2xl">
               <img className="w-full h-[200px]  rounded-2xl object-cover" src={image} alt="" />
               </div>
               <div className="flex justify-between">
               <h3 className="text-2xl font-semibold">Title : {task_title}</h3>
               <h3 className="text-xl font-semibold">Deadline : {completion_date}</h3>
               </div>
               <div className="flex justify-between">
               <h3 className="text-xl font-semibold">💰Earn : {payable_amount}</h3>
               <h3 className="text-xl font-semibold">Available : {required_workers}</h3>
               </div>
                <h4 className="font-semibold">
                ✅ Requirements: <ol className="list-decimal ml-5">
                    <li>
                ...
                <Link to={`vedioplayer/${_id}`} className="button w-full text-center">Watch</Link>
            </div>
          </div>
        </div>
      )}
